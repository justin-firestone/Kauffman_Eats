...
QUnit.test("highlightPath: highlight path 1-2-3", function( assert ) {
    //arrange
    const path = [graphNodes[1], graphNodes[2], graphNodes[3]];
    const color = 'hsla(137, 100%, 50%, 1)';
...
QUnit.test("highlightPath: highlight path 2-4-5", function( assert ) {
    //arrange
    const path = [graphNodes[2], graphNodes[4], graphNodes[5]];
    const color = 'hsla(137, 100%, 50%, 1)';
...
QUnit.test("getOrderedPointsFromPath: points from path 1-2", function( assert ) {
    ...
    //act
    const points = getOrderedPointsFromPath(path);
...
QUnit.test("invertBetweenEdges: flips list of edges [[1,2]]", function(assert){
    //arrange
    const edges = [graphEdges["1_2"]];

    //act
    const inverted = invertEdgesBetween(edges);

    //assert
    assert.deepEqual(edges.length, inverted.length);
    for(let i = 0; i < edges.length; i++){
        const index = edges.length - 1 - i;
...
QUnit.test("invertBetweenEdges: flips list of edges [[1,2],[2,3],[2,4]]", function(assert){
    //arrange
    const edges = [graphEdges["1_2"], graphEdges["2_3"], graphEdges["2_4"]];
...
QUnit.test("getStepsAlongPath: steps for path 1-2", function( assert ) {
    ...
    //assert
    assert.deepEqual(expectedSteps.length, steps.length);
...
QUnit.test("travelDistanceAlongLineSegment: travel distance from 1 to 2", function( assert ) {
    ...
    //act
    const step = travelDistanceAlongLineSegment(start, end, distanceToTravel);

    //assert
    assert.deepEqual(step.x, expectedStep.x);
    assert.deepEqual(step.y, expectedStep.y);
});
